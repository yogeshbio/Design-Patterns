...
//           objects(body, engine) step by step so as to avoid using constructor with many arguments
...
class PlaneBuilder{
protected:
    unique_ptr<Plane> _plane;
public:
    virtual void getPartsDone() = 0;
    virtual void buildBody() = 0;
    virtual void buildEngine() = 0;
    virtual ~PlaneBuilder(){}
    Plane* getPlane(){ return _plane.get(); }
};
...
public:
    void getPartsDone() { _plane = make_unique<Plane>("Propeller Plane"); }
...
public:
    void getPartsDone() { _plane = make_unique<Plane>("Jet Plane"); }
